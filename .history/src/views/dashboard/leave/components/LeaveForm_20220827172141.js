...
function LeaveForm() {
    ...
    const validationSchema = Yup.object().shape({
        leaveStartDate: Yup.date().nullable().required('Date is required'),
        select: Yup.string().required('The type is required'),
        message: Yup.string().required('Message is required'),
    });
    const initialValues = { message: '', select: '', date: null };
    ...
    return (
        <Form onSubmit={handleSubmit} className="tooltip-end-top">
            ...
            <div className="mb-3 filled">
                ...
                <DatePicker className="form-control" name="leaveStartDate" selected={values.startDate} onChange={dateOnChange} placeholderText="Start Date" />
