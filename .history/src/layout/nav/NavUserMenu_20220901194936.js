...
const NavUserMenuDropdownToggle = React.memo(
  React.forwardRef(({ onClick, expanded = false, user = {} }, ref) => {
    ...
    console.log(currentUser.data.user.firstName);
    ...
    return (
      ...
      >
        <img className="profile" alt={currentUser.data.user.photo} src={currentUser.data.user.firstName} />
        <div className="name">{currentUser.data.user.firstName}</div>
      </a>
    )
