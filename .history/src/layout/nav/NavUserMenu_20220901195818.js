...
const NavUserMenuDropdownToggle = React.memo(
  React.forwardRef(({ onClick, expanded = false, user = {} }, ref) => {
    ...
    return (
      ...
      >
        <img className="profile" alt="" src={currentUser.data.user.photo} />
        <div className="name">{currentUser.data.user.firstName} <CsLineIcons icon="chevron-bottom" className="me-2" size="17" />  </div>
      </a>
    )
