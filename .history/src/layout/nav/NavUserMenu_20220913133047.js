...
const NavUserMenuContent = () => {
  ...
  const logout = () => {
    ...
    localStorage.removeItem("persist:starter-project");
  ...
  return (
    <div>
      ...
      <Row className="mb-1 ms-0 me-0">
        ...
        <Col xs="6" className="pe-1 ps-1 cursor-pointer">
          <ul className="list-unstyled">
            <li onClick={logout}>
...
const NavUserMenuDropdownToggle = React.memo(
  React.forwardRef(({ onClick, expanded = false, user = {} }, ref) => {
    ...
    return (
      ...
      >
        <img className="profile" alt={currentUser.data.user.firstName} src={`https://polar-basin-47052.herokuapp.com/img/users/${currentUser.data.user.photo}`} />
